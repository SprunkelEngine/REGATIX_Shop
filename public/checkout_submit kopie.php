...
function pv_validate_discount(string $code, float $grossItems, array $discounts): array {
    ...
    foreach ($discounts as $d) {
        ...
        if ($min > 0 && $grossItems < $min) return ['ok'=>false,'discount'=>0.0,'code'=>$code];
...
if ($mode === 'offer') {
    ...
    // (Optional) du kannst auch hier später Fracht aufnehmen – aktuell: nur Info
    $discountCode = pv_norm_code((string)($_POST['discount_code'] ?? ''));

    $vatRate = (float)($config['import']['vat_rate'] ?? 0.19);

    // Items als Tabelle (ohne Summenlogik)
    ...
    <body style="font-family:Arial,sans-serif;">
      ...
      . ($discountCode !== '' ? '<p><b>Rabattcode:</b> ' . htmlspecialchars($discountCode, ENT_QUOTES, 'UTF-8') . '</p>' : '') . '
      <h3>Positionen</h3>
      ' . $itemsHtml . '
      <hr>
...
$kundenmail = (string)$kundendaten['email'];
$betreiberMail = $config['shop']['contact_email'] ?? ($config['shop']['email'] ?? 'shop@example.com');
...
$ship = [
    ...
    'phone'      => trim((string)($kundendaten['ship_phone'] ?? '')),
...
$orderRecord = [
    ...
    'discount' => [
        'code' => $discountCode,
        'gross' => $discountGross,
        'note' => 'Rabatt nur auf Artikel, nicht auf Fracht',
    ],
    'freight' => [
        'gross' => $freightGross,
        'net'   => $freightNet,
...
if (!empty($ship['use_alt'])) {
    ...
    $shipHtmlAdmin = '<div class="daten"><b>Lieferadresse:</b> Wie Rechnungsadresse</div>';
}
...
<div class="daten">
  ...
  <b>Zahlung:</b> ' . htmlspecialchars($zahlungsart, ENT_QUOTES, 'UTF-8') . '
</div>
' . $shipHtmlAdmin . '
<table class="items">
  <tr>
