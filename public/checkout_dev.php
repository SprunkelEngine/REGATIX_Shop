...
//
// ✅ NEU: Shipping Override (Frachtkosten aus Varianten / shipping.json)
//
$productKey = (string)($boot['product_key'] ?? $boot['productKey'] ?? ($boot['product']['key'] ?? '') ?? '');
$productKey = preg_replace('~[^A-Za-z0-9_\-]~', '', $productKey);

$pvShippingCfg = null;
if ($productKey !== '') {
  $shipPath = __DIR__ . '/../data/products/' . $productKey . '/shipping.json';
  if (is_file($shipPath)) {
	$pvShippingCfg = json_decode((string)file_get_contents($shipPath), true);
	if (!is_array($pvShippingCfg)) $pvShippingCfg = null;
  }
}

//
// ✅ NEU: Shipping-Cost Table (config/shipping_costs.json)
// Struktur: { "fachbodenregal": [ {Land,PLZ,Ort,Versandzone,netto,brutto}, ... ], "sonderposten": [ ... ] }
//
$pvShippingCostCfg = null;
// robust: probiere mehrere mögliche Pfade (je nach DocumentRoot/Ordnerstruktur)
$shipCostCandidates = [
  __DIR__ . '/../config/shipping_costs.json',
  __DIR__ . '/config/shipping_costs.json',
  dirname(__DIR__) . '/config/shipping_costs.json',
  __DIR__ . '/../config/shipping_costs.json',
  __DIR__ . '/config/shipping_costs.json',
];
$shipCostPath = '';
foreach ($shipCostCandidates as $p) {
  if (is_file($p)) { $shipCostPath = $p; break; }
}
if ($shipCostPath !== '') {
  $pvShippingCostCfg = json_decode((string)file_get_contents($shipCostPath), true);
  if (!is_array($pvShippingCostCfg)) $pvShippingCostCfg = null;
}
//
// ✅ Shipping-Cost Table (config/shipping_costs.json)
// Struktur: { "fachbodenregal": [ {Land,PLZ,Ort,Versandzone,netto,brutto}, ... ], "sonderposten": [ ... ] }
//
$pvShippingCostCfg = null;
// robust: probiere mehrere mögliche Pfade (je nach DocumentRoot/Ordnerstruktur)
...
// ✅ Pfade: checkout.php liegt in /public, config liegt in /config
$paths = [
  __DIR__ . '/../config/shipping_costs.json',
  __DIR__ . '/config/shipping_costs.json', // fallback, falls public nicht DocumentRoot ist
];

foreach ($paths as $p) {
  if (is_file($p)) {
	$pvShippingCostCfg = json_decode((string)file_get_contents($p), true);
	if (is_array($pvShippingCostCfg)) break;
	$pvShippingCostCfg = null;
  }
}

// VAT-Rate für Netto-Berechnung (Fallback 19%)
$pvVatRate = (float)($cfg['import']['vat_rate'] ?? $cfg['vat_rate'] ?? 0.19);
...
<div id="logo">
  ...
  <script>
	...
	window.PV_SHIPPING_CFG = <?= json_encode($pvShippingCfg, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES) ?>;
	window.PV_SHIPPING_COST_CFG = <?= json_encode($pvShippingCostCfg, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES) ?>;
	window.PV_SHIPPING_COST_CFG = <?= json_encode($pvShippingCostCfg, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES) ?>;
	window.PV_VAT_RATE = <?= json_encode($pvVatRate) ?>;
...
<script>
/* =========================
   FREIGHT (PRO ARTIKEL IN DER ZUSAMMENFASSUNG)
   - config/shipping_costs.json anhand Produktart + Profil-PLZ/Land (angemeldet)
   - je Artikel Fracht anzeigen + in Summe einrechnen (qty berücksichtigt)
   - Netto > 1.500 € => komplett frachtkostenfrei (Hinweis)
   ========================= */
(function(){
  ...
  function shippingCostCfg(){
	return (window.PV_SHIPPING_COST_CFG && typeof window.PV_SHIPPING_COST_CFG === 'object')
	  ? window.PV_SHIPPING_COST_CFG
	  : null;
  }
  function getVatRate(){
	const r = Number(window.PV_VAT_RATE || 0.19);
	return (isFinite(r) && r >= 0) ? r : 0.19;
  }
  function sanitizeZip(zip, country){
	zip = String(zip || '').trim();
  ...
  function parseOrderJsonAny(){
	const raw =
	  (document.getElementById('pv_buy_order_json')?.value || '') ||
	  (document.getElementById('pv_offer_order_json')?.value || '') ||
	  '';
	if (!raw) return null;
  ...
  function extractItems(order){
	...
	if (order.articles && typeof order.articles === 'object') {
	  const vals = Object.values(order.articles);
	  if (vals.some(v => typeof v === 'object')) return vals;
	}
	return [];
  }

  function detectGroupForItem(it){
	const cfg = shippingCostCfg();
	if (!cfg) return '';
	const keys = Object.keys(cfg);
	if (!keys.length) return '';

	const direct =
	  it?.product_type ?? it?.productType ?? it?.type ??
	  it?.shipping_group ?? it?.shippingGroup ??
	  it?.group ?? it?.category ?? it?.product_group ?? it?.productGroup ??
	  (it?.product && (it.product.type ?? it.product.group ?? it.product.category)) ??
	  '';

	const cand0 = String(direct || '').trim().toLowerCase();
	if (cand0) {
	  for (const g of keys){ if (String(g).toLowerCase() === cand0) return g; }
	  for (const g of keys){
		const gg = String(g).toLowerCase();
		if (cand0.startsWith(gg) || cand0.includes(gg)) return g;
	  }
	}

	const cand = String(
	  it?.product_key ?? it?.productKey ?? it?.key ??
	  it?.sku ?? it?.slug ?? it?.id ??
	  (it?.product && (it.product.key ?? it.product.sku ?? it.product.id)) ??
	  (it?.title ?? it?.name ?? '')
	).trim().toLowerCase();
	...
	for (const g of keys){
	  const gg = String(g).toLowerCase();
	  if (!gg) continue;
	  if (cand.startsWith(gg) || cand.includes(gg)) return g;
	}
	return '';
  }

  function findShippingCostEntry(groupKey, zip, country){
	const cfg = shippingCostCfg();
	if (!cfg) return null;
	const list = cfg[groupKey];
	if (!Array.isArray(list) || !zip || !country) return null;
	...
	for (const row of list){
	  ...
	  const rowC = String(row.Land || row.country || '').trim().toUpperCase();
	  const rowZ = String(row.PLZ  || row.zip     || '').trim();
	  if (rowC === c && rowZ === z) return row;
	}
	return null;
  }
  ...
  const hidNote  = document.getElementById('pv_freight_note');
  ...
  const hidOfferNote  = document.getElementById('pv_offer_freight_note');

  function writeHidden(totalGross, totalNet, zone, note){
	const pickup = isPickupSelected();
	const g = pickup ? 0 : round2(totalGross);
	const n = pickup ? 0 : round2(totalNet);

	if (hidGross) hidGross.value = String(g);
	if (hidNet)   hidNet.value   = String(n);
	if (hidZone)  hidZone.value  = String(zone || '');
	if (hidNote)  hidNote.value  = String(note || '');

	if (hidOfferGross) hidOfferGross.value = String(g);
	if (hidOfferNet)   hidOfferNet.value   = String(n);
	if (hidOfferZone)  hidOfferZone.value  = String(zone || '');
	if (hidOfferNote)  hidOfferNote.value  = String(note || '');
  }

  function getUserProfileReceiver(){
	const u = (typeof window.CURRENT_USER === 'object' && window.CURRENT_USER) ? window.CURRENT_USER : null;
	if (!u) return { zip:'', country:'DE', ok:false };

	const country = String(u.country || 'DE').trim().toUpperCase();
	const zipRaw  = String(u.zip || '').trim();
	const zip     = sanitizeZip(zipRaw, country);

	return { zip, country, ok: !!(zip && country) };
  }

  function baseNetWithoutFreight(){
	const sumNetEl = document.getElementById('pv_sum_net');
	const cur = parseMoney(sumNetEl?.textContent || '');
	const alreadyN = Number(sumNetEl?.dataset?.pvFreightNet || 0) || 0;
	return Math.max(0, cur - alreadyN);
  }

  function applyFreightToTotals(totalGross, totalNet){
	const sumGrossEl = document.getElementById('pv_sum_gross');
	const sumNetEl   = document.getElementById('pv_sum_net');
	if (!sumGrossEl) return;

	const pickup = isPickupSelected();
	const freightG = pickup ? 0 : round2(totalGross || 0);
	const freightN = pickup ? 0 : round2(totalNet   || 0);
	...
	if (sumNetEl) {
	  ...
	  const totalN = baseNet + freightN;
	  sumNetEl.textContent = totalN > 0 ? (formatEUR(totalN) + ' ohne MwSt.') : '';
	}
  }
  ...
  function getProductRows(){
	...
	if (!list) return [];
	// .sum-row pro Position; exclude our optional old freight row if any
	return Array.from(list.querySelectorAll('.sum-row')).filter(r => r.id !== 'pv_freight_row');
  }

  function annotateFreightPerItem(rows, items, perIndex, note){
	const noteStr = String(note || '');
	const isFree = noteStr.startsWith('FREE:');
	const isErr  = noteStr.startsWith('ERR:');
	const msg    = noteStr.replace(/^(FREE:|ERR:)/,'').trim();
	for (let i=0; i<rows.length && i<items.length; i++){
	  const row = rows[i];
	  if (!row) continue;

	  let meta = row.querySelector('.sum-meta');
	  if (!meta) {
		meta = document.createElement('div');
		meta.className = 'sum-meta';
		row.querySelector('.sum-left')?.appendChild(meta);
	  }

	  const old = meta.querySelector('.pv-freight-line');
	  if (old) old.remove();

	  const line = document.createElement('div');
	  line.className = 'pv-freight-line';
	  line.style.marginTop = '4px';

	  if (isPickupSelected()){
		line.textContent = 'Fracht: kostenfrei (Abholung)';
	  } else if (isFree){
		line.textContent = 'Fracht: kostenfrei' + (msg ? (' (' + msg + ')') : '');
	  } else if (isErr){
		line.textContent = 'Fracht: —' + (msg ? (' (' + msg + ')') : '');
	  } else {
		const f = perIndex[i];
		if (f && f.has) {
		  line.textContent =
			'Fracht: ' + formatEUR(f.gross) + ' brutto' +
			(f.net > 0 ? (' · ' + formatEUR(f.net) + ' ohne MwSt.') : '');
		} else {
		  line.textContent = 'Fracht: — (nicht gefunden)';
		}
	  }
  ...
  function computeFreightForItems(items, recv){
	...
	for (const it of items){
	  const qty = Number((it && (it.qty ?? it.quantity ?? it.amount)) ?? 1) || 1;
	  const q = (isFinite(qty) && qty > 0) ? qty : 1;

	  const group = detectGroupForItem(it);
	  const row = group ? findShippingCostEntry(group, recv.zip, recv.country) : null;

	  if (row) {
		const gross1 = Number(row.brutto);
		const net1   = Number(row.netto);

		const gross = isFinite(gross1) ? round2(gross1 * q) : 0;
		const net   = isFinite(net1) ? round2(net1 * q) : (vat > 0 ? round2(gross / (1 + vat)) : 0);

		const zone = String(row.Versandzone || row.zone || group || '').trim();
		if (zone) zones.add(zone);

		perIndex.push({ has:true, gross, net, zone, group, qty:q });
		totalG += gross;
		totalN += net;
	  } else {
		perIndex.push({ has:false, gross:0, net:0, zone:'', group:group || '', qty:q });
	  }
	}

	return { perIndex, totalGross: round2(totalG), totalNet: round2(totalN), zone: Array.from(zones).join(', ') };
  }

  function updateFreight(){
	const order = parseOrderJsonAny();
	const items = extractItems(order);
	if (!items.length) {
	  writeHidden(0,0,'','');
	  applyFreightToTotals(0,0);
	  return;
	}

	// Nicht eingeloggt: keine Fracht rechnen, aber Hinweis pro Artikel
	if (!window.PV_IS_LOGGED_IN) {
	  writeHidden(0,0,'','Bitte anmelden für Frachtkosten.');
	  applyFreightToTotals(0,0);
	  annotateFreightPerItem(getProductRows(), items, [], 'ERR:Bitte anmelden');
	  return;
	}

	if (isPickupSelected()){
	  writeHidden(0,0,'','Abholung – keine Frachtkosten.');
	  applyFreightToTotals(0,0);
	  annotateFreightPerItem(getProductRows(), items, [], 'FREE:Abholung');
	  return;
	}

	// Netto > 1500 => frachtfrei
	const netBase = baseNetWithoutFreight();
	if (netBase > 7000) {
	  writeHidden(0,0,'','Frachtkostenfrei (Netto-Summe über 7.000 €).');
	  applyFreightToTotals(0,0);
	  annotateFreightPerItem(getProductRows(), items, [], 'FREE:Netto > 7.000 €');
	  return;
	}

	const cfg = shippingCostCfg();
	if (!cfg) {
	  writeHidden(0,0,'','shipping_costs.json nicht geladen.');
	  applyFreightToTotals(0,0);
	  annotateFreightPerItem(getProductRows(), items, [], 'ERR:Konfiguration fehlt');
	  return;
	}

	const recv = getUserProfileReceiver();
	if (!recv.ok) {
	  writeHidden(0,0,'','Profil-PLZ fehlt.');
	  applyFreightToTotals(0,0);
	  annotateFreightPerItem(getProductRows(), items, [], 'ERR:Profil-PLZ fehlt');
	  return;
	}

	const calc = computeFreightForItems(items, recv);
	const note = 'Profil-PLZ ' + recv.country + ' ' + recv.zip + (calc.zone ? (' · ' + calc.zone) : '');

	writeHidden(calc.totalGross, calc.totalNet, calc.zone || '', note);
	applyFreightToTotals(calc.totalGross, calc.totalNet);
	annotateFreightPerItem(getProductRows(), items, calc.perIndex, '');
  }

  // öfter, weil checkout.js (Re-)Render macht
  setInterval(updateFreight, 500);
  document.addEventListener('change', (e)=>{
	if (e.target && e.target.matches('input[name="payment"]')) updateFreight();
  });
  setTimeout(updateFreight, 250);
})();
</script>
  ...
  /* =========================
	 ...
	 ✅ FIX: Wenn shipping.json enabled=false → keine Frachtberechnung/Anzeige
	 ✅ NEU: Wenn User eingeloggt → Fracht aus config/shipping_costs.json (PLZ/Land) ziehen
	 ✅ Regel: bei mehreren Posten nur den höchsten Betrag (MAX) nehmen
	 ========================= */
  (function(){
	const form = document.getElementById('pv_buy_form');
	if (!form) return;
	...
	function shippingCfg(){
	  return (window.PV_SHIPPING_CFG && typeof window.PV_SHIPPING_CFG === 'object') ? window.PV_SHIPPING_CFG : null;
	}
	...
	function isProductShippingDisabled(){
	  const cfg = shippingCfg();
	  return !!(cfg && cfg.enabled === false);
	}

	function applyShippingDisabledUI(){
	  if (freightWrap) freightWrap.style.display = 'none';
	  ...
	  outM.textContent = 'Für dieses Produkt sind Frachtkosten deaktiviert.';
	  outB.textContent = '—';
	  outN.textContent = '';
	...
	function ensureFreightWrapVisible(){
	  if (freightWrap) freightWrap.style.display = '';
	}
	...
	function defaultShippingCost(){
	  const cfg = shippingCfg();
	  if (!cfg || !cfg.default || typeof cfg.default !== 'object') return 0;
	  const mode = String(cfg.default.mode || 'preset');
	  if (mode === 'custom') return Number(cfg.default.cost || 0) || 0;
	  const pk = String(cfg.default.preset_key || '');
	  const c = presetCost(pk);
	  return (c === null) ? 0 : c;
	}

	function resolveVariantShippingCost(item){
	  const cfg = shippingCfg();
	  if (!cfg || cfg.enabled === false) return { active:false, cost:0, meta:'' };

	  const s =
		(item && typeof item === 'object' && item.shipping && typeof item.shipping === 'object') ? item.shipping :
		(item && typeof item === 'object' && item.variant && typeof item.variant === 'object' && item.variant.shipping && typeof item.variant.shipping === 'object') ? item.variant.shipping :
		null;

	  const mode = String((s && s.mode) ? s.mode : 'inherit');

	  if (mode === 'free') return { active:true, cost:0, meta:'Fracht: frei (Variante)' };

	  if (mode === 'custom') {
		const c = Number(s && s.cost ? s.cost : 0) || 0;
		return { active:true, cost:Math.max(0,c), meta:'Fracht: Betrag (Variante)' };
	  }

	  if (mode === 'preset') {
		const pk = String(s && s.preset_key ? s.preset_key : '');
		const c = presetCost(pk);
		return { active:true, cost:Math.max(0,(c === null ? 0 : c)), meta:'Fracht: Preset ' + pk };
	  }

	  const c = defaultShippingCost();
	  const meta = (cfg.default && cfg.default.mode === 'preset')
		? ('Fracht: Default Preset ' + String(cfg.default.preset_key || ''))
		: 'Fracht: Default Betrag';
	  return { active:true, cost:Math.max(0,c), meta:meta };
	}
	...
	// ✅ Override aus Varianten/Produkt (MAX-Regel)
	function computeFreightOverride(){
	  const cfg = shippingCfg();
	  if (!cfg || cfg.enabled === false) return { has:false, gross:0, net:0, meta:'' };

	  const order = parseOrderJsonAny();
	  const items = extractItems(order);

	  if (!items.length) {
		return { has:false, gross:0, net:0, meta:'' };
	  }
	  ...
	  for (const it of items){
		const r = resolveVariantShippingCost(it);
	  ...
	  if (!any || !(max >= 0)) return { has:false, gross:0, net:0, meta:'' };

	  const vat = getVatRate();
	  const gross = Math.max(0, max);
	  const net = vat > 0 ? gross / (1 + vat) : 0;

	  metaParts = Array.from(new Set(metaParts)).slice(0, 3);
	  const meta = metaParts.join(' · ');

	  return { has:true, gross:round2(gross), net:round2(net), meta:meta };
	}
	...
	// ✅ Profil-Receiver: aus CURRENT_USER
	function getUserProfileReceiver(){
	  const u = (typeof CURRENT_USER === 'object' && CURRENT_USER) ? CURRENT_USER : null;
	  if (!u) return { zip:'', country:'DE' };

	  const country = String(u.country || 'DE').trim().toUpperCase();
	  const zipRaw  = String(u.zip || '').trim();
	  const zip     = sanitizeZip(zipRaw, country, true);

	  return { zip, country };
	}

	// ✅ Gruppe aus Item-Key ableiten (Prefix match)
	function detectGroupForItem(it){
	  const cfg = shippingCostCfg();
	  if (!cfg) return '';
	  ...
	  const cand = String(
		it?.group || it?.category || it?.product_group ||
		it?.product_key || it?.productKey || it?.key ||
		it?.sku || it?.slug || it?.id || ''
	  ).toLowerCase();
	  ...
	  for (const g of keys){
		...
		if (cand.startsWith(gg)) return g;
		if (cand.includes(gg)) return g;
	  }
	  return '';
	}
	...
	// ✅ Fracht aus shipping_costs.json über Profil-PLZ/Land (MAX-Regel)
	function computeFreightFromUserProfile(){
	  const cfg = shippingCostCfg();
	  if (!cfg) return { has:false, gross:0, net:0, meta:'', zone:'' };

	  const { zip, country } = getUserProfileReceiver();
	  if (!zip || !country) return { has:false, gross:0, net:0, meta:'', zone:'' };

	  const order = parseOrderJsonAny();
	  const items = extractItems(order);
	  if (!items.length) return { has:false, gross:0, net:0, meta:'', zone:'' };
	  ...
	  for (const it of items){
		...
		const row = findShippingCostEntry(group, zip, country);
		if (!row) continue;

		const gross = Number(row.brutto);
		const net   = Number(row.netto);
		if (!isFinite(gross)) continue;

		const zone = String(row.Versandzone || row.zone || '').trim();
		const ort  = String(row.Ort || row.city || '').trim();

		const meta =
		  `Profil-PLZ ${country} ${zip}` +
		  (ort ? ` · ${ort}` : '') +
		  (zone ? ` · ${zone}` : '') +
		  ` · Gruppe: ${group}`;
		...
		const cand = {
		  gross: round2(gross),
		  net:   isFinite(net) ? round2(net) : 0,
		  meta,
		  zone
		};

		if (!best || cand.gross > best.gross) best = cand;
	  }

	  if (!best) return { has:false, gross:0, net:0, meta:'', zone:'' };
	  return { has:true, gross:best.gross, net:best.net, meta:best.meta, zone:best.zone || '' };
	}

	async function updateFreight(force=false){
	  // ✅ HARD STOP: Produkt hat Fracht deaktiviert → gar nichts rechnen/anzeigen
	  if (isProductShippingDisabled()){
		applyShippingDisabledUI();
		return;
	  } else {
		ensureFreightWrapVisible();
	  }

	  if (PV_PICKUP) { applyAll(); return; }

	  // ✅ 1) Profil-Fracht (config/shipping_costs.json) – MAX-Regel
	  if (window.PV_IS_LOGGED_IN) {
		const up = computeFreightFromUserProfile();
		if (up.has) {
		  PV_FREIGHT_FOUND = true;
		  PV_FREIGHT_GROSS = up.gross;
		  PV_FREIGHT_NET   = up.net;
		  PV_FREIGHT_ZONE  = up.zone || 'profil';
		  PV_FREIGHT_META  = up.meta || 'Fracht nach Profil-PLZ';

		  outM.textContent = PV_FREIGHT_META;
		  outB.textContent = formatEUR(PV_FREIGHT_GROSS) + ' brutto';
		  outN.textContent = PV_FREIGHT_NET > 0 ? (formatEUR(PV_FREIGHT_NET) + ' ohne MwSt.') : '';
		  applyAll();
		  return;
		}
	  }

	  // ✅ 2) Override-Frachtkosten aus Varianten / shipping.json – MAX-Regel
	  const ov = computeFreightOverride();
	  if (ov.has) {
		PV_FREIGHT_FOUND = true;
		PV_FREIGHT_GROSS = ov.gross;
		PV_FREIGHT_NET   = ov.net;
		PV_FREIGHT_ZONE  = 'override';
		PV_FREIGHT_META  = ov.meta || 'Frachtkosten aus Variante/Produkt-Override';
		outM.textContent = 'Frachtkosten (Override) angewendet.';
		outB.textContent = formatEUR(PV_FREIGHT_GROSS) + ' brutto';
		outN.textContent = PV_FREIGHT_NET > 0 ? (formatEUR(PV_FREIGHT_NET) + ' ohne MwSt.') : '';
		applyAll();
		return;
  ...
  <div id="footer"><p>REGATIX Betriebseinrichtungen GmbH &bull; Porschestra&szlig;e 9 &bull; 74360 Ilsfeld &bull; Telefon: 07062 - 23 902 - 0 &bull; E-Mail: info@regatix.com<br />
	 Montag - Donnerstag: 08:00 - 12:00 Uhr | 13:00 - 17:00 Uhr &bull; Freitag:08:00 - 12:00 Uhr | 13:00 - 16:30 Uhr  <br><a href="https://www.regatix.com/pages/start/impressum.php" target="_blank">Impressum</a> | <a href="https://www.regatix.com/pages/start/datenschutz.php" target="_blank">Datenschutz</a> | <a href="https://www.regatix.com/pages/start/versandbedingungen.php" target="_blank">Versandbedingungen</a> | <a href="https://www.regatix.com/pages/start/widerrufsrecht.php" target="_blank">Widerruf</a> | <a href="https://www.regatix.com/pages/start/shop-bedingungen.php" target="_blank">SHOP Bedingungen</a></p>
  </div>
</body>
</html>
