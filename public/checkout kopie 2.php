		...
		<div class="card-b">
		  ...
		  <form method="post" action="checkout_submit.php" id="pv_buy_form" autocomplete="off">
			...
			<div class="row">
			  ...
			  <div><label>Telefon für die Zustellung</label><input type="tel" name="phone" required value=""></div>
			</div>
			<div style="height:10px"></div>
...
<!-- Alternative Lieferadresse (aufklappbar) -->
			<div class="alt-ship-wrap" style="margin-top:10px;">
			  ...
			  <div id="pv_ship_alt_box" style="display:none; margin-top:10px; padding:12px; border:1px solid var(--border); border-radius:14px; background:rgba(0,0,0,.02);">
				<div class="row">
				  ...
				  <div><label>Telefon (optional)</label><input type="tel" name="ship_phone" value=""></div>
				</div>
				<div style="height:10px"></div>
 ...
 (function(){
  ...
  (function(){
	...
	const hid   = document.getElementById('pv_discount_code');
	const hidOffer = document.getElementById('pv_offer_discount_code');
	...
	async function applyCode(){
	  ...
	  try{
		...
		if (!res.ok || !data.ok) {
		  setMsg(data.error || 'Rabattcode ungültig.', false);
		  return;
		}

		if (hid) hid.value = data.code || code;
		if (hidOffer) hidOffer.value = data.code || code;

		setMsg(
		  'Code akzeptiert: ' + (data.code || code) +
		  ' · Rabatt: ' + (data.discount_gross || '0.00') + ' €',
		  true
		);
  ...
  <div id="modal_register" class="modal">
	<div class="modal-content">
	  ...
	  <form id="form_register" autocomplete="off">
		...
		<input type="tel"  name="phone" placeholder="Telefon (optional)" value=""><br>
