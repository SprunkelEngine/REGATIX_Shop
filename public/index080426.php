...
function pv_compact_key(string $s): string
{
    $s = pv_lower(trim($s));
...
function pv_is_palettenregal_product(string $productKey, string $label = '', string $title = ''): bool
{
    $all = pv_compact_key($productKey . ' ' . $label . ' ' . $title);

    return str_contains($all, 'palettenregal')
        || str_contains($all, 'palettenregalgrundregal');
}

function pv_palettenregal_variants_path(): string
{
    return __DIR__ . '/../data/products/Palettenregale_V2/variants.json';
}

function pv_load_palettenregal_source_variants(): array
{
    $path = pv_palettenregal_variants_path();
...
function pv_load_palettenregal_price_map(): array
{
    $variants = pv_load_palettenregal_source_variants();
    $out = [];
    ...
    foreach ($variants as $row) {
        ...
        $out[trim($article)] = [
            'gross' => $gross !== null ? trim($gross) : '',
            'net'   => $net !== null ? trim($net) : '',
        ];
    }
...
function pv_enrich_boot_prices_from_palettenregal_source(array $boot, string $productKey, string $label = '', string $title = ''): array
{
    if (!pv_is_palettenregal_product($productKey, $label, $title)) {
        return $boot;
    }

    if (empty($boot['variants']) || !is_array($boot['variants'])) {
        return $boot;
    }

    $priceMap = pv_load_palettenregal_price_map();

    if ($priceMap === []) {
        return $boot;
    }

    $variantCfg = isset($boot['config']['variant']) && is_array($boot['config']['variant'])
        ? $boot['config']['variant']
        : [];

    $primaryKey = (string)($variantCfg['primary_key'] ?? 'Artikelnummer');
    $priceGrossKey = (string)($variantCfg['price_gross'] ?? 'mit MwSt. €');
    $priceNetKey = (string)($variantCfg['price_net'] ?? 'ohne MwSt. €');

    foreach ($boot['variants'] as $idx => $row) {
        if (!is_array($row)) {
            continue;
        }
        ...
        if (!isset($priceMap[$article]) || !is_array($priceMap[$article])) {
            continue;
        }

        $existingGross = trim((string)($row[$priceGrossKey] ?? ''));
        $existingNet = trim((string)($row[$priceNetKey] ?? ''));

        if ($existingGross === '' && ($priceMap[$article]['gross'] ?? '') !== '') {
            $boot['variants'][$idx][$priceGrossKey] = $priceMap[$article]['gross'];
        }

        if ($existingNet === '' && ($priceMap[$article]['net'] ?? '') !== '') {
            $boot['variants'][$idx][$priceNetKey] = $priceMap[$article]['net'];
        }
    }
...
function pv_strip_duplicate_image_prefix(string $path): string
{
    ...
    $baseUrl = rtrim(pv_product_image_base_url(), '/');
    $baseUrlQuoted = preg_quote($baseUrl, '~');

    for ($i = 0; $i < 10; $i++) {
        $old = $path;

        $path = preg_replace(
            '~^(?:https?://[^/]+/images/)+(https?://.+)$~i',
            '$1',
            $path
        );

        $path = preg_replace(
            '~^(?:' . $baseUrlQuoted . '/)+(https?://.+)$~i',
            '$1',
            $path
        );
...
function pv_normalize_image_path(string $path): string
{
    ...
    $path = pv_strip_duplicate_image_prefix($path);
...
$boot = pv_enrich_boot_prices_from_palettenregal_source($boot, $productKey);
...
$boot = pv_enrich_boot_prices_from_palettenregal_source($boot, $productKey, $currentLabel, $title);
...
>
    ...
    <div class="container">
        <div class="header">
            ...
            <div style="display:flex; gap:10px; flex-wrap:wrap; align-items:center">
                ...
                <a class="pill-toggle" href="cart.php" title="Warenkorb öffnen">
                    Warenkorb
                    <span id="pv_cart_badge" class="pv-badge" hidden></span>
                </a>
                ...
                <div class="grid">
                    <div>
                        ...
                        <div class="card" id="pv_info_card" style="border-radius:var(--radius2)">
                            <div class="card-h">
                                <div class="card-title">Infotext</div>
                            </div>
                    ...
                    <div>
                        ...
                        <div class="card" style="border-radius:var(--radius2)">
                            <div class="card-h">
                                <div class="card-title">Infodaten</div>
                            </div>
    ...
    <div id="footer">
        <p>
            ...
            <a href="https://www.regatix.com/pages/start/widerrufsrecht.php" target="_blank">Widerrufsrecht</a> |
    ...
    <script>
        (function(){
            ...
            function stripDuplicateImagePrefix(img){
                img = String(img == null ? '' : img).trim().replace(/\\/g, '/');
                if(!img) return '';
                ...
                for(let i = 0; i < 10; i++){
                    ...
                    img = img.replace(/^(?:https?:\/\/[^/]+\/images\/)+(https?:\/\/.+)$/i, '$1');

                    if(escapedBase){
                        const rx = new RegExp('^(?:' + escapedBase + '/)+(https?:\\/\\/.+)$', 'i');
                        img = img.replace(rx, '$1');
                    }
            ...
            function normUrl(img){
                img = stripDuplicateImagePrefix(img);
                if(!img) return '';
    ...
    <script>
        (function(){
            ...
            function renderInfo(row){
                ...
                addRow('Nennhöhe', getRowValue(['Nennhöhe', 'Nennhoehe', 'Nenn Höhe', 'Nenn-Höhe', 'Höhe', 'Hoehe', 'Nennhöhe mm']));
