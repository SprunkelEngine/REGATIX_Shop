...
<div id="logo">
	  ...
	  <div class="card">
		...
		<div class="card-b">
		  ...
		  <form method="post" action="checkout_submit.php" id="pv_buy_form" autocomplete="off">
			...
			<input type="hidden" name="freight_zone"  id="pv_freight_zone" value="0">
			...
			<div class="row">
			  ...
			  <div><span><strong><label><span style="color:#c0392b;">Telefon für die Zustellung (zwingend)&nbsp;</span></label></strong></span><input name="ship_phone" type="tel" value="" required /></div>
			</div>
			<div style="height:10px"></div>
			...
			<div class="alt-ship-wrap" style="margin-top:10px;">
			  ...
			  <div id="pv_ship_alt_box" style="display:none; margin-top:10px; padding:12px; border:1px solid var(--border); border-radius:14px; background:rgba(0,0,0,.02);">
				<div class="row">
				  ...
				  <div><span><strong><label><span style="color:#c0392b;">Telefon für die Zustellung (zwingend)&nbsp;</span></label></strong></span><input name="ship_phone_alt" type="tel" value="" /></div>
				</div>
				<div style="height:10px"></div>
 ...
 (function(){
   ...
   const mainPhoneEl = form ? form.querySelector('input[name="ship_phone"]') : null;
   const altPhoneEl  = form ? form.querySelector('input[name="ship_phone_alt"]') : null;
   const altToggleEl = document.getElementById('pv_ship_alt_toggle');

   let ppActionsRef = null;

   function activePhoneEl(){
	 return (altToggleEl && altToggleEl.checked) ? altPhoneEl : mainPhoneEl;
   }
   ...
   function phoneMissingMessage(){
	 return (altToggleEl && altToggleEl.checked)
	   ? 'Bitte Telefonnummer der alternativen Lieferadresse eintragen, dann ist die Bezahlmethode verfügbar.'
	   : 'Bitte Telefon für die Zustellung eintragen, dann ist die Bezahlmethode verfügbar.';
   }
   ...
   function syncPayPalPhoneGate(){
	 ...
	 if (!ppActionsRef) {
	   if (!phoneOk() && ppMsg) ppMsg.textContent = phoneMissingMessage();
	 ...
	 if (phoneOk()){
	   ...
	   if (ppMsg) ppMsg.textContent = phoneMissingMessage();
	 }
   }

   [mainPhoneEl, altPhoneEl].forEach(el => {
	 if (!el) return;
	 ['input','change','blur'].forEach(evt => el.addEventListener(evt, syncPayPalPhoneGate));
   });

   if (altToggleEl) {
	 altToggleEl.addEventListener('change', syncPayPalPhoneGate);
   }
   ...
   form.addEventListener('submit', function(e){
	 ...
	 if (!phoneOk()) {
	   e.preventDefault();
	   if (ppMsg) ppMsg.textContent = phoneMissingMessage();
	   alert('Bitte geben Sie eine Telefonnummer für die Zustellung ein.');
	   activePhoneEl()?.focus();
	   syncPayPalPhoneGate();
	   return;
	 }
 ...
 (function(){
  ...
  <div id="modal_register" class="modal">
	<div class="modal-content">
	  ...
	  <form id="form_register" autocomplete="off">
		...
		<div style="display:flex; gap:14px; align-items:center; flex-wrap:wrap; margin:6px 0 10px 0;">
		  <label style="display:flex; gap:8px; align-items:center; cursor:pointer;">
			<input type="checkbox" id="reg_type_private" name="type_private" value="1">
			Privatkunde
		  </label>

		  <label style="display:flex; gap:8px; align-items:center; cursor:pointer;">
			<input type="checkbox" id="reg_type_company" name="type_company" value="1">
			Firmenkunde
		  </label>

		  <input type="hidden" name="customer_type" id="reg_customer_type" value="">
		</div>

		<input type="text" name="company" placeholder="Firma (optional)" value=""><br>

		<div id="reg_vat_wrap" style="display:none;">
		  <input type="text" name="vat_id" id="reg_vat_id" placeholder="USt-IdNr. (zwingend für Firmenkunde)" value=""><br>
		</div>

		<input type="tel"  name="phone" placeholder="Telefon für die Zustellung (zwingend)" required value=""><br>
  ...
  <div id="modal_profile" class="modal">
	<div class="modal-content">
	  ...
	  <form id="form_profile" autocomplete="off">
		...
		<div style="display:flex; gap:14px; align-items:center; flex-wrap:wrap; margin:0 0 10px 0;">
		  <label style="display:flex; gap:8px; align-items:center; cursor:pointer;">
			<input type="checkbox" id="profile_type_private" name="type_private" value="1">
			Privatkunde
		  </label>

		  <label style="display:flex; gap:8px; align-items:center; cursor:pointer;">
			<input type="checkbox" id="profile_type_company" name="type_company" value="1">
			Firmenkunde
		  </label>

		  <input type="hidden" name="customer_type" id="profile_customer_type" value="">
		</div>
		...
		<div id="profile_vat_wrap" style="display:none;">
		  <label>USt-IdNr.</label>
		  <input type="text" name="vat_id" id="profile_vat_id" value="">
		</div>
		<div style="height:10px"></div>
  ...
  <!-- NEU: Modal für fehlende Telefonnummer -->
  <div id="modal_phone_required" class="modal">
	<div class="modal-content">
	  <button class="close" type="button" id="close_phone_required">&times;</button>
	  <h2>Telefonnummer erforderlich</h2>
	  <div style="margin-top:10px; line-height:1.5;">
		Es ist zwingend notwendig eine Telefonnummer einzutragen um eine reibungslose Zustellung zu garantieren!
	  </div>
	  <div style="margin-top:18px;">
		<button class="btn" type="button" id="btn_phone_required_ok" style="width:100%; height:44px; font-weight:800;">
		  OK
		</button>
	  </div>
	</div>
  ...
  <script>
	...
	document.getElementById('close_phone_required')?.addEventListener('click', ()=> hideModal('modal_phone_required'));
	document.getElementById('btn_phone_required_ok')?.addEventListener('click', ()=> hideModal('modal_phone_required'));
	...
	function clearRegisterForm(){
	  ...
	  const priv = document.getElementById('reg_type_private');
	  const comp = document.getElementById('reg_type_company');
	  const vatWrap = document.getElementById('reg_vat_wrap');
	  const vatEl = document.getElementById('reg_vat_id');
	  const typeHid = document.getElementById('reg_customer_type');

	  if (priv) priv.checked = false;
	  if (comp) comp.checked = false;
	  if (typeHid) typeHid.value = '';
	  if (vatEl) { vatEl.value = ''; vatEl.required = false; }
	...
	function syncCustomerTypeUI(prefix){
	  const priv = document.getElementById(prefix + '_type_private');
	  const comp = document.getElementById(prefix + '_type_company');
	  const vatWrap = document.getElementById(prefix + '_vat_wrap');
	  const vatEl = document.getElementById(prefix + '_vat_id');
	  const typeHid = document.getElementById(prefix + '_customer_type');

	  if (!priv || !comp || !vatWrap || !vatEl || !typeHid) return;

	  const isCompany = !!comp.checked;
	  vatWrap.style.display = isCompany ? 'block' : 'none';
	  vatEl.required = isCompany;
	  typeHid.value = isCompany ? 'company' : (priv.checked ? 'private' : '');

	  if (!isCompany) vatEl.value = '';
	}
	...
	(function(){
	  ...
	  if (priv && comp) {
		priv.addEventListener('change', () => {
		  if (priv.checked) comp.checked = false;
		  syncCustomerTypeUI('reg');
		});
		comp.addEventListener('change', () => {
		  if (comp.checked) priv.checked = false;
		  syncCustomerTypeUI('reg');
		});
		syncCustomerTypeUI('reg');
	  }
	...
	(function(){
	  const priv = document.getElementById('profile_type_private');
	  const comp = document.getElementById('profile_type_company');
	  if (priv && comp) {
		priv.addEventListener('change', () => {
		  if (priv.checked) comp.checked = false;
		  syncCustomerTypeUI('profile');
		});
		comp.addEventListener('change', () => {
		  if (comp.checked) priv.checked = false;
		  syncCustomerTypeUI('profile');
	...
	btnProfile?.addEventListener('click', async ()=>{
	  ...
	  if(f){
		...
		const pPriv = document.getElementById('profile_type_private');
		const pComp = document.getElementById('profile_type_company');
		const pVat  = document.getElementById('profile_vat_id');

		const cType = String(CURRENT_USER.customer_type || '').trim().toLowerCase();
		if (pPriv) pPriv.checked = (cType === 'private');
		if (pComp) pComp.checked = (cType === 'company');
		if (pVat) pVat.value = String(CURRENT_USER.vat_id || '');
	...
	document.getElementById('form_register').addEventListener('submit', async (e)=>{
	  ...
	  const isPriv = !!document.getElementById('reg_type_private')?.checked;
	  const isComp = !!document.getElementById('reg_type_company')?.checked;
	  const vat    = (document.getElementById('reg_vat_id')?.value || '').trim();

	  if (!isPriv && !isComp) {
		if (errEl) errEl.textContent = 'Bitte wählen Sie Privatkunde oder Firmenkunde.';
		return;
	  }
	  if (isComp && !vat) {
		if (errEl) errEl.textContent = 'Bitte USt-IdNr. eintragen (Pflicht für Firmenkunde).';
		return;
	  }
	...
	document.getElementById('form_profile')?.addEventListener('submit', async (e)=>{
	  ...
	  const isPriv = !!document.getElementById('profile_type_private')?.checked;
	  const isComp = !!document.getElementById('profile_type_company')?.checked;
	  const vat    = (document.getElementById('profile_vat_id')?.value || '').trim();

	  if (!isPriv && !isComp) {
		if(msg){ msg.textContent = 'Bitte wählen Sie Privatkunde oder Firmenkunde.'; msg.classList.add('err'); }
		return;
	  }
	  if (isComp && !vat) {
		if(msg){ msg.textContent = 'Bitte USt-IdNr. eintragen (Pflicht für Firmenkunde).'; msg.classList.add('err'); }
		return;
	  }
...
<script>
  (function(){
	...
	const OFFER_NET_THRESHOLD = 1500;
	const FREE_FREIGHT_NET_THRESHOLD = 7000;

	const offerBox = document.getElementById('pv_offer_box');
	...
	function syncOfferBox(){
	  if (!offerBox) return;
	  const show = (!PV_PICKUP && PV_BASE_NET > OFFER_NET_THRESHOLD);
	  offerBox.style.display = show ? '' : 'none';
	}

	function writeHidden(){
	  const active = (PV_FREIGHT_FOUND && !PV_PICKUP && !PV_FREE_FREIGHT);

	  const g = active ? PV_FREIGHT_GROSS : 0;
	  const n = active ? PV_FREIGHT_NET   : 0;
	  const z = active ? (PV_FREIGHT_ZONE || '') : '';
	...
	function renderFreightRow(){
	  ...
	  const visible = (!PV_PICKUP && (PV_FREIGHT_FOUND || PV_FREE_FREIGHT));
	  ...
	  if (PV_FREE_FREIGHT) {
		if (g) g.textContent = formatEUR(0) + ' brutto';
		if (n) n.textContent = '';
		if (m) m.textContent = 'Frachtkostenfrei ab netto 7.000 €.';
		return;
	  }
	...
	function applyFreightToTotals(){
	  ...
	  if (sumNetEl) {
		...
		baseNet = Math.max(0, currentNet - alreadyN);
	  }

	  PV_BASE_NET = baseNet;
	  PV_FREE_FREIGHT = (!PV_PICKUP && PV_BASE_NET >= FREE_FREIGHT_NET_THRESHOLD);
	  ...
	  if (PV_FREE_FREIGHT) {
		outM.textContent = 'Frachtkostenfrei ab netto 7.000 €.';
		outB.textContent = formatEUR(0) + ' brutto';
		outN.textContent = '';
	  }
...
<script>
 (function(){
   ...
   const mainPhoneEl = form.querySelector('input[name="ship_phone"]');
   const altPhoneEl  = form.querySelector('input[name="ship_phone_alt"]');
   const altToggleEl = document.getElementById('pv_ship_alt_toggle');
   ...
   [mainPhoneEl, altPhoneEl].forEach(el => {
	 ...
	 el.addEventListener('blur', syncSubmit);
   });
   ...
   form.addEventListener('submit', function(e){
	 ...
	 if (!selectedPayment()) {
	   e.preventDefault();
	   alert("Bitte wählen Sie eine Bezahlmethode.");
	   return;
	 }
   });
...
<!-- NEU: Zahlungsarten blockieren und Modal zeigen, wenn keine Telefonnummer vorhanden ist -->
<script>
(function(){
  const form = document.getElementById('pv_buy_form');
  if (!form) return;
  ...
  paymentRadios.forEach(radio => {
	radio.addEventListener('change', function(e){
	  const blockedPayments = ['paypal', 'bank_transfer', 'cash_pickup'];

	  if (!blockedPayments.includes(this.value)) return;

	  if (!phoneOk()) {
		this.checked = false;
		resetPaymentSelection();
		showModal('modal_phone_required');
		activePhoneEl()?.focus();
		e.preventDefault();
	  }
	});
...
<script>
 (function(){
   ...
   const requiredNames = new Set([
	 'ship_first_name','ship_last_name','ship_street','ship_zip','ship_city','ship_country','ship_phone_alt'
   ]);
