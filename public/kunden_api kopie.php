...
function lower($s): string { return mb_strtolower((string)$s, 'UTF-8'); }
...
/**
 * ✅ Pfad zu users.json
 * In deinem Fall liegt sie unter /data/users.json (URL: https://regatix.shop/data/users.json)
 * => Server-Pfad ist typischerweise: __DIR__ . '/../data/users.json' (public/.. = root)
 */
 ...
 * WICHTIG: Damit NICHT /public im URL auftaucht:
 * Setze in config.json:
 *   shop.base_url = "https://regatix.shop"
 */
...
/**
 * ✅ Mail senden – bessere Deliverability (aber kein 100% Anti-Spam)
 */
function send_mail_better(string $to, string $subjectUtf8, string $bodyTextUtf8, array $shop, ?string &$debug=null): bool {
  ...
  // RFC2047 subject encoding (UTF-8)
  ...
  // Envelope sender / Return-Path über -f (hilft oft gegen Spam)
  $params = '';
...
try {
  ...
  if ($action === 'whoami') {
    ...
    if (!is_array($u) || empty($u['email'])) out(['ok'=>false], 200);
  ...
  if ($action === 'register') {
    ...
    if ($email === '' || !filter_var($email, FILTER_VALIDATE_EMAIL)) out(['ok'=>false,'error'=>'Ungültige E-Mail.'], 400);
    if (strlen($pw) < 6) out(['ok'=>false,'error'=>'Passwort zu kurz (min. 6 Zeichen).'], 400);

    $users = load_users($GLOBALS['usersPath']);

    if (isset($users[$email])) out(['ok'=>false,'error'=>'E-Mail existiert bereits.'], 409);

    $first = htrim($_POST['first_name'] ?? '');
    $last  = htrim($_POST['last_name'] ?? '');
    if ($first === '' || $last === '') out(['ok'=>false,'error'=>'Vorname/Nachname fehlt.'], 400);
  ...
  if ($action === 'login') {
    ...
    if ($email === '' || $pw === '') out(['ok'=>false,'error'=>'E-Mail oder Passwort fehlt.'], 400);
    ...
    // session user (ohne password_hash)
    $sess = $u;
  ...
  if ($action === 'update_profile') {
    ...
    if (!is_array($sess) || empty($sess['email'])) out(['ok'=>false,'error'=>'Nicht angemeldet.'], 401);

    $email = lower((string)$sess['email']);
    $users = load_users($GLOBALS['usersPath']);
    if (!isset($users[$email]) || !is_array($users[$email])) out(['ok'=>false,'error'=>'User nicht gefunden.'], 404);
    ...
    if (($u['first_name'] ?? '') === '' || ($u['last_name'] ?? '') === '') out(['ok'=>false,'error'=>'Vorname/Nachname fehlen.'], 400);
  ...
  if ($action === 'reset_request') {
    $email = lower(htrim($_POST['email'] ?? ''));
    // absichtlich generische Antwort (User Enumeration verhindern)
    $generic = ['ok'=>true, 'message'=>'Wenn die E-Mail existiert, wurde ein Reset-Link versendet.'];

    if ($email === '' || !filter_var($email, FILTER_VALIDATE_EMAIL)) out($generic, 200);
    ...
    // Hash speichern (niemals Token im JSON)
    ...
    // DEV-Link anzeigen, falls Mail nicht geht oder wenn du es explizit willst:
  ...
  if ($action === 'reset_password') {
    ...
    if ($email === '' || $token === '' || $newPw === '') out(['ok'=>false,'error'=>'Ungültige Anfrage.'], 400);
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) out(['ok'=>false,'error'=>'Ungültige E-Mail.'], 400);
    if (strlen($newPw) < 6) out(['ok'=>false,'error'=>'Passwort zu kurz (min. 6 Zeichen).'], 400);

    $users = load_users($GLOBALS['usersPath']);
    if (!isset($users[$email]) || !is_array($users[$email])) out(['ok'=>false,'error'=>'Ungültiger Link.'], 400);

    $u = $users[$email];
    $hash = (string)($u['password_reset_token_hash'] ?? '');
    $exp  = (int)($u['password_reset_expires'] ?? 0);

    if ($hash === '' || $exp <= 0 || time() > $exp) out(['ok'=>false,'error'=>'Token abgelaufen. Bitte neu anfordern.'], 400);

    $calc = hash('sha256', $token);
    if (!hash_equals($hash, $calc)) out(['ok'=>false,'error'=>'Ungültiger Token.'], 400);
    ...
    // Optional: Session NICHT automatisch einloggen, nur bestätigen.
    out(['ok'=>true], 200);
  }
